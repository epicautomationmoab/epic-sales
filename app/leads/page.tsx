...
export default async function LeadsPage() {
  ...
  let error = "";
  try {
    const payload = await loadOpenLeads(accessToken);
    leads = payload.leads || [];
  } catch (err) {
    error = err instanceof Error ? err.message : "Unable to load Sales leads.";
  }
      ...
      <section className={styles.main}>
        <header className={styles.header}>
          <div>
            ...
            <h1>Open Leads</h1>
            <p>The same live sales opportunities your team has already been working.</p>
          </div>
