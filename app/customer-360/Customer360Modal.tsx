  ...
  return <div className={styles.backdrop} onMouseDown={e=>{if(e.currentTarget===e.target)onClose();}}><section className={styles.modal} role="dialog" aria-modal="true" aria-label="Customer 360">
    ...
    {loading?<div className={styles.loading}>Loading Customer 360…</div>:error?<div className={styles.error}>{error}</div>:!data?<div className={styles.empty}>No customer record found.</div>:<div className={styles.body}>
      ...
      <aside className={styles.sidebar}>
        ...
        <div className={styles.actionGrid}><a href="/quote" className={styles.actionCard}><span>Quote</span><strong>Build Quote</strong></a>{identity.email?<a href={`mailto:${identity.email}`} className={styles.actionCard}><span>Reach Out</span><strong>Send Email</strong></a>:<div className={styles.actionCardDisabled}><span>Reach Out</span><strong>Add Email</strong></div>}{identity.phone?<a href={`sms:${identity.phone}`} className={styles.actionCard}><span>Reach Out</span><strong>Send Text</strong></a>:<div className={styles.actionCardDisabled}><span>Reach Out</span><strong>Add Phone</strong></div>}{identity.tripworks_customer_code&&twCustomerUrl(identity.tripworks_customer_code)?<a href={twCustomerUrl(identity.tripworks_customer_code)!} target="_blank" rel="noreferrer" className={styles.actionCard}><span>Booking</span><strong>Book in TripWorks</strong></a>:<div className={styles.actionCardDisabled}><span>Booking</span><strong>No TW Customer ID</strong></div>}</div>
