...
type EventItem={id:string;kind:"call"|"text"|"email"|"reservation"|"lead"|"draft"|"quote"|"note";at:string|null;title:string;meta?:string|null;body?:string|null;href?:string|null;hrefLabel?:string|null;transcript?:string|null};
...
export default function Customer360Modal(props:Props){
  ...
  const timeline=useMemo<EventItem[]>(()=>{if(!data)return[];const items:EventItem[]=[];
    ...
    for(const o of data.opportunities||[])items.push({id:`o-${o.id}`,kind:"lead",at:o.shopping_last_activity_at||o.updated_at,title:o.status==="booked"?"Lead booked":o.status==="open"?"Active lead":`Lead ${o.status||""}`.trim(),meta:[o.interest_label,o.lead_value_cents!=null?money.format(o.lead_value_cents/100):null,o.claimed_by_name||o.assigned_rep_name].filter(Boolean).join(" · "),body:o.activity_window_start?`Visit window ${fmtDate(o.activity_window_start)}${o.activity_window_end&&o.activity_window_end!==o.activity_window_start?` – ${fmtDate(o.activity_window_end)}`:""}`:null});
    for(const d of data.drafts||[]){
      const state=d.converted_at?"Converted to booking":d.is_current_draft===false?"Closed":"Open draft";
      items.push({id:`d-${d.id}`,kind:"draft",at:d.converted_at||d.last_seen_at,title:`${d.converted_at?"TripWorks draft converted":"TripWorks draft"}: ${d.experience_name||"Draft Reservation"}`,meta:[d.option_name,d.value_cents!=null?money.format(d.value_cents/100):null,d.confirmation_code,state].filter(Boolean).join(" · "),body:d.activity_date?`For ${fmtDate(d.activity_date)}`:null,href:twReservationUrl(d.confirmation_code)||undefined,hrefLabel:"Open in TripWorks"});
    }
  ...
  if(openLeads.length)badges.push(`${openLeads.length} Active Lead${openLeads.length===1?"":"s"}`);
  ...
  return <div className={styles.backdrop} onMouseDown={e=>{if(e.currentTarget===e.target)onClose();}}><section className={styles.modal} role="dialog" aria-modal="true" aria-label="Customer 360">
    ...
    {loading?<div className={styles.loading}>Loading Customer 360…</div>:error?<div className={styles.error}>{error}</div>:!data?<div className={styles.empty}>No customer record found.</div>:<div className={styles.body}>
      <main className={styles.journey}><div className={styles.lifecycleHeader}><div><div className={styles.eyebrow}>Customer Lifecycle</div><h3>Customer Lifecycle</h3></div><span>{timeline.length} events</span></div><div className={styles.filters}>{([['all','All'],['call','Calls'],['text','Texts'],['email','Emails'],['other','Other']] as Array<[LifecycleFilter,string]>).map(([value,label])=><button key={value} className={filter===value?styles.filterActive:styles.filter} onClick={()=>setFilter(value)}>{label}</button>)}</div><div className={styles.timeline}>{visibleTimeline.length?visibleTimeline.map(item=><article key={item.id} className={`${styles.event} ${styles[item.kind]}`}><div className={styles.eventTop}><strong>{item.title}</strong><span>{fmtDateTime(item.at)}</span></div>{item.meta?<div className={styles.eventMeta}>{item.meta}</div>:null}{item.body?<div className={styles.eventBody}>{item.body}</div>:null}{item.href?<a href={item.href} target={item.href.startsWith("http")?"_blank":undefined} rel={item.href.startsWith("http")?"noreferrer":undefined}>{item.hrefLabel||"Open"}</a>:null}{item.kind==="call"&&item.transcript?<CallTranscript transcript={item.transcript}/>:null}</article>):<div className={styles.emptyTimeline}>No activity in this view yet.</div>}</div></main>
      <aside className={styles.sidebar}><div className={styles.commandTitle}><div className={styles.eyebrow}>Current Customer Context</div><h3>Command Center</h3></div>
        {activeReservations.length?<div className={styles.sideSection}><h3>Current / Upcoming Reservation</h3>{activeReservations.map(r=><div key={r.id} className={styles.record}><div className={styles.recordTitle}>{r.experience_name||r.confirmation_code}</div><div className={styles.recordMeta}>{[r.confirmation_code,r.start_time?fmtDateTime(r.start_time):null,r.trip_status,r.amount_due_cents!=null?`${money.format(r.amount_due_cents/100)} due`:null].filter(Boolean).join(" · ")}</div><div className={styles.recordActions}>{twReservationUrl(r.confirmation_code)?<a className={styles.button} href={twReservationUrl(r.confirmation_code)!} target="_blank" rel="noreferrer">Open Reservation</a>:null}</div></div>)}</div>:null}
        {openLeads.length?<div className={styles.sideSection}><h3>Active Lead</h3>{openLeads.map(o=><div key={o.id} className={styles.record}><div className={styles.recordTitle}>{o.interest_label||"Active sales opportunity"}</div><div className={styles.recordMeta}>{[o.lead_value_cents!=null?money.format(o.lead_value_cents/100):null,o.activity_window_start?fmtDate(o.activity_window_start):null,o.claimed_by_name||o.assigned_rep_name||"Unclaimed"].filter(Boolean).join(" · ")}</div></div>)}</div>:null}
        ...
        {openLeads.length&&futureDrafts.length?<div className={styles.sideSection}><h3>Current Draft Activity</h3><div className={styles.compactRecord}><strong>{futureDrafts.length} TripWorks draft{futureDrafts.length===1?"":"s"}</strong><span>{money.format(futureDrafts.reduce((s,d)=>s+(d.value_cents||0),0)/100)} in current draft options</span></div></div>:null}
