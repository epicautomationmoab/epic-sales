...
export default function InboxClient(){
  const[threads,setThreads]=useState<InboxThread[]>([]);const[selectedKey,setSelectedKey]=useState<string|null>(null);const[filter,setFilter]=useState<Filter>("open");const[query,setQuery]=useState("");const[busy,setBusy]=useState(false);const[error,setError]=useState("");const[customer360Open,setCustomer360Open]=useState(false);
  ...
  useEffect(()=>{void load("open");const timer=window.setInterval(()=>{if(document.visibilityState==="visible")void load(filter);},5000);return()=>window.clearInterval(timer);},[filter]);
  ...
  return <>
    ...
    <div className={styles.workspace}>
      ...
      <section className={styles.detailPane}>{selected?<div className={styles.card}><div className={styles.detailTop}><div><div className={styles.eyebrow}>{selected.lane==="sales"?"Sales opportunity":selected.lane==="service"?"Existing customer":"Needs identification"}</div><h2>{selected.customer_name||selected.email||selected.phone||"Unknown customer"}</h2><div className={styles.sub}>{[selected.email,selected.phone].filter(Boolean).join(" · ")||"No known contact record yet"}</div></div><span className={`${styles.pill} ${styles[selected.lane]}`}>{selected.lane}</span></div>
        ...
        <div className={styles.actions}><button className={styles.secondary} onClick={()=>setCustomer360Open(true)}>Open Customer 360</button>{selected.opportunity_id?<a className={styles.secondary} href={`/leads?open=${encodeURIComponent(selected.opportunity_id)}`}>Open Lead</a>:null}<button className={styles.primary} disabled={busy||!selected.is_open} onClick={()=>void clean()}>{busy?"Cleaning…":selected.is_open?"Clean / Handled":"Already Cleaned"}</button></div><div className={styles.cleanNote}>Cleaning removes this conversation from the active inbox. Any new inbound call, text, email, or shopping activity after that point automatically brings it back.</div>
